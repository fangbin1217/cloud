    ...
    public function actionBind() {
        ...
        if ($access_token) {
            ...
            if ($cache) {
                $cacheList = json_decode($cache, true);
                $expireTime = 2;
                $lockKey = 'BIND_LOCK:'.$cacheList['id'];
                $getLock = false;
                //加锁 防止并发
                $now = time();
                //获取锁
                if (!Yii::$app->redis->setnx($lockKey, $now + $expireTime)) {
                    //锁过期
                    if ($now > Yii::$app->redis->get($lockKey) && $now > Yii::$app->redis->getset($lockKey, $now + $expireTime)) {
                        $getLock = true;
                        //重置锁
                        Yii::$app->redis->setnx($lockKey, $now + $expireTime);
                    }
                } else {
                    //第一次得到锁
                    $getLock = true;
